...
| 遮挡传感器后 `DEMO/backend/seat_state.db` 数据不变 | 正常现象：当前固件对接主系统 `/api/sensor/report`，不写离线演示用的 `seat_state.db`；离线演示见 `DEMO/README.md`，主系统数据在 MySQL `seat_navigation`（或 fallback `seat_navigation.db`） |
