...
> 关键开关（固件 `DEMO/src/main.cpp` 顶部 `IR_ACTIVE_HIGH`）：
> - **HC-SR501（PIR，检测到人体输出 HIGH）→ 填 `true`**（你现在用它）
> - **红外避障模块（检测到物体输出 LOW）→ 填 `false`**

| ESP32 引脚 | 传感器 A（→ `ir_front`） | 传感器 B（→ `ir_back`） |
| --- | --- | --- |
| `GPIO23` | HC-SR501 `OUT` / 红外避障 `OUT` | — |
| `GPIO27` | — | HC-SR501 `OUT` / 红外避障 `OUT` |
| `5V`（或 `3.3V`） | 传感器 `VCC` | 传感器 `VCC` |
| `GND` | 传感器 `GND` | 传感器 `GND` |

- **红外避障**：两传感器**交叉放置**（座椅左前 45° + 右后 45°），人坐下时两束同时被遮 → `(1,1)`。
- **HC-SR501（PIR）**：检测到**人体移动**时输出 HIGH。建议两个传感器一个朝座椅前侧、一个朝后侧（各约 45°）；**只用 1 个**时，把它的 `OUT` **同时接到 `GPIO23` 和 `GPIO27`**，令 `ir_front==ir_back`。
  > PIR 识别的是“移动”而非“静止占座”。请把 HC-SR501 的**延时电位器调到最小**、跳线选 **H（可重复触发）**、灵敏度调到覆盖座位即可，并远离热源/气流/阳光（详见本文第 10 节）。
- **HC-SR04P（超声波测距，推荐替代）**：改为**测距离**——人在桌位附近、距离小于阈值即判占用（阈值在管理面板「设备管理」里可调）。
  | HC-SR04P 引脚 | 连到 ESP32 |
  |---|---|
  | `VCC` | `3.3V`（HC-SR04P 是 3.3V 版，不用 5V） |
  | `TRIG` | `GPIO25` |
  | `ECHO` | `GPIO26` |
  | `GND` | `GND` |
  - 在面板把该设备“传感器类型”选 **HC-SR04P(超声波)**、设**距离阈值(cm)**（如 50，小于该值视为有人）→ 保存。
  - 固件会发 TRIG 脉冲、读 ECHO 脉宽换算距离，与阈值比较上报 `(1,1)`/`(0,0)`；后端状态机不变。
